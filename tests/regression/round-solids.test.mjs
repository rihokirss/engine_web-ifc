...
for (const schema of ['ifc2x3','ifc4','ifc4x3_add2']) {
 for (const [name,volume] of [['sphere',32*Math.PI/3],['cylinder',16*Math.PI],['circle',16*Math.PI],['hollow-circle',7*Math.PI],['ellipse',8*Math.PI]]) {
  test(`${schema}: closed ${name}`,async()=>check(fs.readFileSync(new URL(`./geometry-fixtures/${schema}-${name}.ifc`,import.meta.url),'utf8'),volume));
 }
}
test('sphere placement is applied exactly once',async()=>{
 const text=fs.readFileSync(new URL('./geometry-fixtures/ifc4-sphere.ifc',import.meta.url),'utf8').replace('#13=IFCCARTESIANPOINT((0.,0.,0.));','#13=IFCCARTESIANPOINT((10.,20.,30.));');
